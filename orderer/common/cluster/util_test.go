...
func TestConfigFromBlockBadInput(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			conf, err := replication.ConfigFromBlock(testCase.block)
			require.Nil(t, conf)
			require.Error(t, err)
	...
	for _, testCase := range []struct {
		...
		*/
		...
		t.Run(testCase.description, func(t *testing.T) {
			...
			mockPolicyManager.GetPolicyReturns(testCase.policy, true)
			verifier := &replication.BlockValidationPolicyVerifier{
...
func TestBlockVerifierAssembler(t *testing.T) {
	...
	t.Run("Good config envelope", func(t *testing.T) {
		bva := &replication.BlockVerifierAssembler{BCCSP: cryptoProvider}
...
func TestVerificationRegistryRegisterVerifier(t *testing.T) {
	...
	registry := &cluster.VerificationRegistry{
		Logger:             flogging.MustGetLogger("test"),
		VerifiersByChannel: make(map[string]replication.BlockVerifier),
		VerifierFactory:    verifierFactory,
	}

	var loadCount int
	registry.LoadVerifier = func(chain string) replication.BlockVerifier {
...
func TestVerificationRegistry(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		expectedVerifier      replication.BlockVerifier
		verifierFromConfig    replication.BlockVerifier
